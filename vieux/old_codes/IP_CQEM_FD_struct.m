function [u1, distances, out_spect, shapes_time, nf] =  IP_CQEM_FD_struct(u0,dt,L,dz,alpha,betap,gamma,wzdw,wzdw_photon,tol,option)
...
% wzdw - central frequency of the simulation (THz) [ZDW-> frequency]
% wzdw_photon - central frequency of the simulation (THz) [ZDW-> frequency for photon loss error]
% tol - relative photon error
% option - 2: saves time intermediate steps
%
% OUTPUTS
%
...
while propagedlength < L,
    ...
    PhotonN = sum( (abs(ufft).^2)./(w' + wzdw_photon) );
    
    halfstep = exp(linearoperator*dz/2);
    uip = halfstep.*ufft;

    k1 = -dz*1i*gamma*(1 + w'/wzdw).*fft( u1.*((1-fr)*abs(u1).^2) + fr*dt*u1.*ifft(hrw.*fft( abs(u1).^2 )));
    k1 = halfstep.*k1;
    
    uhalf2 = ifft(uip + k1/2);
    k2 = -dz*1i*gamma*(1 + w'/wzdw).*fft( uhalf2.*((1-fr)*abs(uhalf2).^2) + fr*dt*uhalf2.*ifft(hrw.*fft( abs(uhalf2).^2 )));
    
    uhalf3 = ifft(uip + k2/2);
    k3 = -dz*1i*gamma*(1 + w'/wzdw).*fft( uhalf3.*((1-fr)*abs(uhalf3).^2) + fr*dt*uhalf3.*ifft(hrw.*fft( abs(uhalf3).^2 )));
    
    uhalf4 = ifft(halfstep.*(uip + k3));
    k4 = -dz*1i*gamma*(1 + w'/wzdw).*fft( uhalf4.*((1-fr)*abs(uhalf4).^2) + fr*dt*uhalf4.*ifft(hrw.*fft( abs(uhalf4).^2 )));
    ...
    error = abs(   sum( (abs(uaux).^2)./(w' + wzdw_photon)) - PhotonN   ) / PhotonN;
